...
def build(root, version):
    ...
    try:
        ...
        # include binaries, license texts and some source files for the license information
        binaries = glob('compiler/icud*') + glob('compiler/icuu*') + glob('_deps/icu-src/LICENSE')
        binaries += glob('license/*') + ['compiler/grcompiler', 'compiler/stddef.gdh', 'compiler/LZ4/lz4hc.h']
        binaries += ['preprocessor/gdlpp', 'preprocessor/memory.c', 'preprocessor/README']
